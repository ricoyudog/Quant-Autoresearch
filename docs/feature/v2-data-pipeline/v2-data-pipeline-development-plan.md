...
> Planning status: Sprint 2 execution in progress; Sprint 2 Steps 1-2 closed out
...
| Sprint 2 -- Strategy + Backtester | Add the dual-method strategy interface and minute-level walk-forward pipeline | updated `backtester.py`, updated `active_strategy.py`, trading-day windows | in progress | continue Step 3 in `sprint2/sprint2-backend.md` |
...
| Task ID | Task | Lane | Dependency | Effort | Status | Acceptance |
| --- | --- | --- | --- | --- | --- | --- |
| CLI-01 | Update `fetch`, `backtest`, and `update_data` CLI flows to the DuckDB/minute-data model | Backend | Sprint 2 complete | 0.6d | pending | the CLI exposes the intended runtime behavior with stable arguments |
