...
class CacheConnector:
    ...
    def fetch_and_cache(self, symbol: str, start_date: str = "2020-01-01") -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to fetch/cache {symbol}: {exc}")
            return False
    ...
    def save_data(self, symbol: str, df: pd.DataFrame, format: str = "parquet") -> None:
        clean_symbol = symbol.replace("-", "_")
    ...
    def ingest_custom_csv(self, file_path: str, symbol: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to ingest custom CSV {file_path}: {exc}")
            return False
